...
class Solution:
    def closestCost(
        ...
        @cache
        def solve(currCost, index):
            curr_diff = abs(currCost - target)

            if curr_diff < self.diff:
                self.diff = curr_diff
                self.ans = currCost
            elif curr_diff == self.diff:
                self.ans = min(self.ans, currCost)

            if index == m:
                return

            for count in range(3):
                solve(currCost + toppingCosts[index] * count, index + 1)
